// ==UserScript==
// @name         Sipulse Omnipresente v3.2
// @namespace    http://tampermonkey.net/
// @version      3.2
// @description  Cópia automática de Protocolo e Atendimento 100% Oculto
// @match        *://*/*
// @grant        window.focus
// @grant        GM_setValue
// @grant        GM_getValue
...
(function() {
    ...
    document.getElementById('omni-btn-ligar').addEventListener('click', () => {
        if(visorLigar.value.length > 0) {
            ...
            GM_setValue('omni_estado_chamada', { status: 'ativa', numero: visorLigar.value, inicio: Date.now(), protocolo: '', copiado: false });
    ...
    // === COPIAR PROTOCOLO MANUALMENTE (ETIQUETA AMARELA) ===
    document.getElementById('omni-ativa-protocolo').addEventListener('click', function() {
        ...
        GM_setClipboard(protocolo); // Usa a função forte do Tampermonkey
    ...
    if (isSipulseTab) {
        ...
        setInterval(() => {
            ...
            if (textoDaTela.includes("Recebendo chamada de")) {
                ...
                if (estadoAtual.status !== 'tocando') {
                    ...
                    if (Notification.permission === "granted" && !notificacaoJaDisparada) {
                        ...
                        // Notificação Silenciosa (Sem puxar a aba)
                        const notif = new Notification("⚠️ LIGAÇÃO ENTRANDO!", { body: "Clique para atender sem sair da sua tela.", requireInteraction: true });
            ...
            // DETETA CHAMADA ATIVA
                ...
                // 🕵️‍♂️ AUTO-COPIAR O PROTOCOLO GHOST
            ...
            // Envia as atualizações para todas as abas
            if (houveMudanca) {
    ...
    GM_addValueChangeListener('omni_estado_chamada', (nome, antigo, novo) => {
        ...
        const visorTransfer = document.getElementById('omni-visor-transfer');
        ...
        if (novo.status === 'tocando') {
            ...
            // GESTÃO DO PROTOCOLO NO PAINEL
            if (novo.protocolo && novo.protocolo !== '') {
                protoDiv.innerText = 'PROTOCOLO: ' + novo.protocolo;
                protoDiv.style.display = 'inline-block';

                // Preenche o Visor de Transferência
                if(visorTransfer.value === '') {
                    visorTransfer.value = novo.protocolo;
                    visorTransfer.parentElement.classList.add('ativo-amarelo');
                    document.getElementById('container-visor-ligar').classList.remove('ativo-azul');
                    inputAtivo = visorTransfer;
                }
            } else {

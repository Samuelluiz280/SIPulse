// ==UserScript==
// @name         Sipulse Omnipresente: Central de Comando 3.1
// @namespace    http://tampermonkey.net/
// @version      3.1
// @description  Info de Chamada (Tempo/Numero), Limpador, Busca e Anti-Clones
// @match        *://*/*
// @grant        window.focus
// @grant        GM_setValue
// @grant        GM_getValue
...
(function() {
    ...
    estilo.innerHTML = `
        ...
        /* NOVA SESSÃO: CHAMADA ATIVA (Tempo e Número) */
    ...
    document.getElementById('omni-btn-ligar').addEventListener('click', () => {
        if(visorLigar.value.length > 0) {
            ...
            GM_setValue('omni_estado_chamada', { status: 'ativa', numero: visorLigar.value, inicio: Date.now() });
    ...
    if (isSipulseTab) {
        ...
        setInterval(() => {
            ...
            // LÓGICA DO CRONÓMETRO E ESTADO DE CHAMADA
            ...
            const estadoAtual = GM_getValue('omni_estado_chamada', { status: 'livre', numero: '', inicio: 0 });
            ...
            if (textoDaTela.includes("Recebendo chamada de")) {
                ...
                if(estadoAtual.status !== 'tocando') {
                    GM_setValue('omni_estado_chamada', { status: 'tocando', numero: caller, inicio: 0 });
                }

                if (Notification.permission === "granted" && !notificacaoJaDisparada) {
                    notificacaoJaDisparada = true;
                    const notif = new Notification("⚠️ LIGAÇÃO ENTRANDO!", { body: "Clique para atender.", requireInteraction: true });
                    notif.onclick = function() { window.focus(); const btn = document.querySelector('button#call'); if (btn) btn.click(); this.close(); };
                    setTimeout(() => { notificacaoJaDisparada = false; }, 10000);
                }
            }
            else if (btnEncerrarNaTela) {
                // Se existe o botão de desligar, a chamada está ativa
                if(estadoAtual.status !== 'ativa') {
                    let num = estadoAtual.numero || "Em Andamento";
                    GM_setValue('omni_estado_chamada', { status: 'ativa', numero: num, inicio: Date.now() });
                }
            }
            else {
                // Sistema Livre
                if(estadoAtual.status !== 'livre') {
                    GM_setValue('omni_estado_chamada', { status: 'livre', numero: '', inicio: 0 });
                    GM_setValue('omni_estado_mute', { mutado: false, ts: Date.now() }); // Zera o mute
                }
            }
        }, 1000);
        ...
        GM_addValueChangeListener('omni_comando', (nome, antigo, novo) => {
            if (novo.acao === 'atender') {
                const btn = document.querySelector('button#call'); if (btn) { btn.click(); }
                // Atualiza o estado imediatamente
                const est = GM_getValue('omni_estado_chamada', {});
                GM_setValue('omni_estado_chamada', { status: 'ativa', numero: est.numero, inicio: Date.now() });
            }
    ...
    GM_addValueChangeListener('omni_estado_chamada', (nome, antigo, novo) => {
        ...
        clearInterval(intervalCronometro); // Limpa o relógio anterior
        ...
        if (novo.status === 'tocando') {
            ...
            telaChamada.style.display = 'block'; painel.style.display = 'block';
            ...
            infoChamada.style.display = 'block'; // MOSTRA O BANNER DE INFO
            document.getElementById('omni-ativa-numero').innerText = novo.numero;

            // Balão fica Verde Neon
            ...
            telaChamada.style.display = 'none'; telaTeclado.style.display = 'block'; barraBusca.style.display = 'block';
            infoChamada.style.display = 'none'; // ESCONDE O BANNER
    ...
    GM_addValueChangeListener('omni_estado_mute', (nome, antigo, novo) => {
        atualizarBotaoMute(novo.mutado);
    });

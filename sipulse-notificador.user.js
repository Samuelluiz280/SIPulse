...
(function() {
    ...
    function ocultarElementosNativos() {
        ...
        const botoesLigarNativos = Array.from(document.querySelectorAll('button')).filter(btn => {
            if (btn.id === 'omni-btn-ligar' || btn.id === 'omni-btn-atender') return false;
            if (btn.classList.contains('btn-call')) return true;
            const icon = btn.querySelector('mat-icon');
            return icon && icon.innerText.trim() === 'call';
        });
        botoesLigarNativos.forEach(btn => {
        ...
        const botoesDesligarNativos = Array.from(document.querySelectorAll('button')).filter(btn => {
            if (btn.id === 'omni-btn-desligar') return false;
            const icon = btn.querySelector('mat-icon');
            return icon && icon.innerText.trim() === 'call_end';
        });
        botoesDesligarNativos.forEach(btn => {
            if (btn.style.display !== 'none') {
                btn.style.setProperty('display', 'none', 'important');
            }
        });
    ...
    if (isSipulseTab) {
        ...
        setInterval(ocultarElementosNativos, 1000);
        ...
        setInterval(() => {
            ...
            const matchProtocolo = textoDaTela.match(/(?:N\.?º?|Nº)?\s*(?:Atendimento|Protocolo):\s*(\d+)/i);
    ...
    function injetarInterface() {
        ...
        estilo.innerHTML = `
            ...
            /* Botão Desligar/Transferência Vibrante e Vivo (Vermelho Neon) */
            #omni-btn-desligar { background-color: #ff3b30; box-shadow: 0 4px 10px rgba(0,0,0,0.7), 0 0 18px rgba(255, 59, 48, 0.4); border: 1px solid rgba(255, 59, 48, 0.9);}
            #omni-btn-desligar:hover { background-color: #ff4f44; box-shadow: 0 0 25px rgba(255, 59, 48, 0.8); border-color: #ff6b6b;}
            #omni-btn-desligar:active { transform: scale(0.92); }

            /* Botão X para apagar e desligar (Apenas se não estiver em ligação ativa) */
            #omni-btn-apagar { background-color: rgba(0,0,0,0.6); border: 1px solid rgba(255, 255, 255, 0.2); color:rgba(255,255,255,0.9); font-weight:bold; font-size:14px; backdrop-filter: blur(6px); transition: all 0.2s;}
            ...
            <div id="omni-painel">
                ...
                <div id="omni-tela-teclado">
                    ...
                    <div class="omni-linha-acao">
                        <button id="omni-btn-apagar" class="omni-btn-acao" title="Apagar visor / Desligar discagem">X</button>
        ...
        // ❌ EVENTO DO BOTÃO APAGAR (Com lógica do Notepad da Chamada para desligamento seguro)
        document.getElementById('omni-btn-apagar').addEventListener('click', () => {
            // Verifica a presença do balão de Notepad na tela
            const estaEmConversacaoAtiva = document.body.innerText.toLowerCase().includes("notepad da chamada");

            if (!estaEmConversacaoAtiva) {
                // Se NÃO está em ligação ativa com o cliente (está apenas discando ou tocando), o botão X cancela a chamada diretamente
                GM_setValue('omni_comando', { acao: 'desligar_manual', ts: Date.now() });

                const toast = document.createElement('div');
                toast.innerText = "❌ Cancelando/Desligando chamada...";
                ...
                // Também limpa o visor e foca nele
                inputAtivo.value = '';
                inputAtivo.focus();
            } else {
                // Se a ligação já está ativa e em andamento (balão do Notepad está visível), o X funciona apenas como backspace/limpar visor normal
                inputAtivo.value = '';
                inputAtivo.focus();
            }
        });
